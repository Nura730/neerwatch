...
export function Sidebar({ pendingCount, isOpen, onClose }) {
  ...
  return (
    <nav
      className={[
        'fixed inset-y-0 left-0 z-50 w-[240px] bg-slate-900 border-r border-slate-800 flex flex-col shrink-0 text-slate-300',
      ...
      <div className="flex-1 py-4 overflow-y-auto space-y-6">
        ...
        {showOperations && (
          <div>
            ...
            {canCreateTest(role) && (
              <NavItem to="/observations/new" icon={Plus} label="Create Test" onClick={onClose} />
            )}
            {canSyncTests(role) && (
              <NavItem
                to="/sync"
        ...
        {showAdmin && (
          <div>
            ...
            <NavItem to="/users" icon={Users} label="User Management" onClick={onClose} />
          </div>
        )}
      </div>
